...
public class ReadOrdersFile {
	private Orders_List ord_list= new Orders_List();
	public void loadFile(String data) {
        ...
        try {
			...
			if (line!=null) {
                if (line.trim().equalsIgnoreCase("ORDERS_LIST")) {
				    line = reader.readLine();
                    ...
                    if (line != null) {
                        if (line.trim().equals("{")) { 
						    ...
						    while (line != null && !line.startsWith("}")) {
                                    ...
                                    if (line.trim().toUpperCase().startsWith("ORDER")) {
                                        line = reader.readLine();
                                        ...
                                        if (line !=null) {
										    if (line.trim().equals("{")) {
												...
												reader.reset();
												while (true) { //ORDER_NUMBER
												    line = reader.readLine();
													if (line!=null) {
														if (line.trim().toUpperCase().startsWith("ORDER_NUMBER")) {
															ord.setCodeOfOrder(Integer.parseInt(line.trim().substring(12).trim()));
															break;
														}
			                                        }
												}
												...
												while (true) { //CUSTOMER
													...
													if (line!=null) {
														if (line.trim().toUpperCase().startsWith("CUSTOMER")) {
															((Orders_and_Sales) ord).setCustomer(line.trim().substring(9).trim());
															break;
														}
                                                    }
												}
												...
												while (true) { //PHONE
													...
													if (line!=null) {
														if (line.trim().toUpperCase().startsWith("PHONE")) {
															((Orders_and_Sales) ord).setPhone(line.trim().substring(6).trim());
															break;
														}
                                                    }
												}
												reader.reset();
												while (true) { //ORDER_DATE
													line = reader.readLine();
													if (line!=null) {
														if (line.trim().toUpperCase().startsWith("ORDER_DATE")) {
															((Orders_and_Sales) ord).setDate(line.trim().substring(11).trim());
															break;
														}
                                                    }
												}
												reader.reset();
												while (true) { //DELIVERY_DATE
													line = reader.readLine();
													if (line!=null) {
														if (line.trim().toUpperCase().startsWith("DELIVERY_DATE")) {
															ord.setArrival_Date(line.trim().substring(14).trim());
															break;
														}
                                                    }
												}
												...
												while (true) { //FINAL_PRICE
													...
													if (line!=null) {
													    if (line.trim().toUpperCase().startsWith("FINAL_PRICE")) {
															((Orders_and_Sales) ord).setFinal_Price(Double.parseDouble(line.trim().substring(12).trim()));
															break;
														}
	                                                }
												}
												reader.reset();
												while (true) { //ORDER_STATUS
													line = reader.readLine();
													if (line!=null) {
														if (line.trim().toUpperCase().startsWith("ORDER_STATUS")) {
															ord.setStatus(line.trim().substring(13).trim());
															break;
														}
                                                    }
												}
	...
	public Orders_List getOrd_List() {
		return ord_list;
	}
}
